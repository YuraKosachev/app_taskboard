...
public class TaskSelectUnitImpl
        extends DbExecutorQueryUnit<List<Task>, TaskPredicate>
        implements SelectUnit<TaskPredicate, Task> {
    public TaskSelectUnitImpl(DbConnector connector) {
        super(connector);
    }

    @Override
    public List<Task> getAll(TaskPredicate predicate) {
    ...
    protected PreparedStatement getPreparedStatement(Connection connection, TaskPredicate model) throws SQLException {
        String sql = "SELECT id, title, description, descriptionHtml, priority, status, createdAt FROM %s".formatted(DbConstants.DB_TABLE_TASK);
